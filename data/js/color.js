...
color_htm += '</tr>';
color_htm += '</table>';
color_htm += '<div id="xqcms_color_show"></div>';
function color_show(id, color, obj) {
	if($('#xqcms_color').length==0) {
		var xqcms_color_div = document.createElement("div");
		with(xqcms_color_div.style) {zIndex = 9999; position = 'absolute'; display = 'none'; width = '160px'; padding = '1px'; top = 0; left = 0; border = '#A0A0A0 1px solid'; backgroundColor = '#FFFFFF';}
		xqcms_color_div.id = 'xqcms_color';
		document.body.appendChild(xqcms_color_div);
	}
	var aTag = obj; var leftpos = toppos = 0;
	do {aTag = aTag.offsetParent; leftpos	+= aTag.offsetLeft; toppos += aTag.offsetTop;
	} while(aTag.offsetParent != null);
	$('#xqcms_color').css('left',(obj.offsetLeft + leftpos));
	$('#xqcms_color').css('top' ,(obj.offsetTop + toppos + 20));
	$('#xqcms_color').html(color_htm);
	color_id = id;
	color_bk = color;
	$('#xqcms_color').css('display', '');
	if(color) color_view(color);
	color_setup(color);
}
function color_hide() {Dh('xqcms_color');}
function color_close() {color_hide(); $('#color_img_'+color_id).css('backgroundColor',color_bk);}
function color_select(color) {$('#color_input_'+color_id).val(color); $('#color_img_'+color_id).css('backgroundColor',color); color_hide();}
function color_setup(color) {
	...
	colors_select += '</table>';
	$('#xqcms_color_show').html(colors_select);
}
function color_view(color) {try {$('#color_viewview').val(color); $('#color_viewview').css('color', color); $('#color_img_'+color_id).css('backgroundColor', color);} catch(e) {}}