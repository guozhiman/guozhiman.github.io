    ...
    <div class="m_l">
        <div style="width:250px;">
             <div class="title1"><h2>分类导航 <em>Category Navigation</em></h2></div>
              <div class="bboby">
                <ul>
             <?php $arr=type("moduleid=$moduleid&catid=$CAT[parentid]&ismenu=1"); if(is_array($arr)){ foreach($arr as $i =>$row){ ?>
              <li class="newslist"><a  
  href="<?php echo $row['url'];?>"><?php echo set_style($row['catname'],$row['color']);?></a> </li>
  <?php } } unset($arr);?>	
                </ul>
            </div>
                        <?php if($CAT['parentid']==12) { ?>
                                <div class="title2"><h2>热门点击 <em>Top Click</em></h2></div>
        <div class="bboby">
                <ul>
                <?php $arr=lists("moduleid=$moduleid&condition=status=3&catid=$catid&pagesize=6&order=hits desc"); if(is_array($arr)){ foreach($arr as $i =>$row){ ?>
              <li class="newslist"><a  
